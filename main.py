...
bot = Client("bot",
             bot_token= "6008450783:AAEpx9Pj-tVSzU43-PaI",
             api_id= 2776,
             api_hash= "6a9806661f4b091")
auth_users = [59580,-1078824]
...
async def txt_handler(bot: Client, m: Message):
    ...
    try:
        for i in range(arg, len(links)):
            ...
            try:                               
                cc = f'** {str(count).zfill(3)}.** {name1} ({res}) Brodha_V.mkv\n**Batch Name :** {b_name}\n\n**Downloaded by : {CR}**'
                cc1 = f'** {str(count).zfill(3)}.** {name1} Brodha_V.pdf \n**Batch Name :**{b_name}\n\n**Downloaded by : {CR}**'
                if "drive" in url:
                    try:
